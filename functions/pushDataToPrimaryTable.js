 ...
 */
async function pushDataToPrimaryTable(data) {
    const batchSize = Number(process.env.BATCHSIZE) || 2000;

    try {
        // Split data into batches
        for (let i = 0; i < data.length; i += batchSize) {
            // await sharedResource.writeData(data[i]);
            ...
            try {
                ...
                for (const entry of batch) {
                    ...
                    values.push(`,,0,${DateTime},${Value},${tagKey},`);
                    // placeholders.push('(?, ?, ?)'); // Each row has 3 placeholders
                }

                // bulk write to csv file
                await sharedResource.writeData(values);
